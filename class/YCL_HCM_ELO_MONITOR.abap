CLASS ycl_hcm_elo_monitor DEFINITION
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized" lang="pt">Processa os dados que foram exportados</p>
    METHODS process_exported .
    "! <p class="shorttext synchronized" lang="pt">Processa os dados que foram importados</p>
    METHODS process_imported .
...
*       p_subty    =
  ...
  METHOD set_export_message .
    ...
    IF ( line_exists( log[ type = if_xo_const_message=>error ] ) ) . " Retornou erro?
  ...
  METHOD export_data .
    ...
    IF ( line_exists( me->messages[ type = if_xo_const_message=>error ] ) ) .
      RETURN .
    ENDIF .
...
*     selections->set_selection_mode( if_salv_c_selection_mode=>cell ) .
  ...
  METHOD search_data .
    ...
    SELECT h~id, h~gjahr, h~bukrs, h~pernr,
     ...
     WHERE h~bukrs EQ @filter-empresa
       AND h~type  IN @range_infotipos
       AND ( ( i~start_date IN @me->filter-data ) OR
             ( i~end_date   IN @me->filter-data ) )
  ...
  METHOD process_exported .
  ENDMETHOD .


  METHOD process_imported .
  ENDMETHOD .
